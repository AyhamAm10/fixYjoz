...
const AddressCart:React.FC<props> = ({data , refetch}) => {
  const [position, _setPosition] = useState<LatLngLiteral | null>(null);
  ...
  const handleDeleteAddress =async ()=>{
    try {
      axiosClaint.post(endPoints.post.deletedAddress , {address_id : data.id} , 
        {
          headers: {
  ...
  return (
    <div className="w-[21rem] sm:w-[18rem] min-h-[21rem] py-6 px-3 rounded-md shadow-lg flex flex-col gap-6 ">
      <div className="w-full text-[1rem] text-red font-semibold flex items-center justify-between">
        <h3 className="text-dark">{data.label}</h3> <h3>{data.is_default ? "default" : ""}</h3>
      </div>
      <p className="text-sm font-normal text-[#B0B0B0] pr-4">
        Lorem ipsum dolor sit amet consectetur. Velit ut malesuada id.
      </p>
